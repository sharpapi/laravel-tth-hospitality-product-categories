 ...
 * @api
 */
class TthHospitalityProductCategoriesService extends SharpApiClient
{
    /**
    ...
    {
        parent::__construct(config('sharpapi-tth-hospitality-product-categories.api_key'));
        $this->setApiBaseUrl(
            config(
                'sharpapi-tth-hospitality-product-categories.base_url',
                'https://sharpapi.com/api/v1'
            )
        );
        $this->setApiJobStatusPollingInterval(
            (int) config(
                'sharpapi-tth-hospitality-product-categories.api_job_status_polling_interval',
                5)
        );
        $this->setApiJobStatusPollingWait(
            (int) config(
                'sharpapi-tth-hospitality-product-categories.api_job_status_polling_wait',
                180)
        );
        $this->setUserAgent('SharpAPILaravelTthHospitalityProductCategories/1.0.0');
    }

    /**
     * Generates a list of suitable categories for the Hospitality product
    ...
    public function hospitalityProductCategories(
        ...
        $response = $this->makeRequest(
            'POST',
            '/tth/hospitality_product_categories',
            [
                'content' => $productName,
                'city' => $city,
                'country' => $country,
